...
from calculate import calculate_discount, calculate_material

#для получения файлов .py
...
class MainWindow(tk.Tk):
    ...
    def init_materials_tab(self):
        ...
        self.materials_table = ttk.Treeview(frame, columns=("ID", "Тип", "Название", "Поставщик", "Количество", "Ед. изм."), show="headings")
        headers = ["ID", "Тип", "Название", "Поставщик", "Количество", "Ед. изм."]
        for header in headers:
            self.materials_table.heading(header, text=header)
            self.materials_table.column(header, width=100)
        self.materials_table.grid(row=0, column=0, sticky="nsew")

        scrollbar = ttk.Scrollbar(frame, orient="vertical", command=self.materials_table.yview)
        self.materials_table.configure(yscrollcommand=scrollbar.set)
        scrollbar.grid(row=0, column=1, sticky="ns")

        button_frame = ttk.Frame(frame)
        button_frame.grid(row=1, column=0, columnspan=2, pady=10)
        ttk.Button(button_frame, text="Обновить", command=self.load_materials).grid(row=0, column=0, padx=10)
    ...
    def load_materials(self):
        try:
            if not table_exists(self.db_file, 'materials'):
                messagebox.showerror("Ошибка", "Таблица 'materials' не существует.", parent=self)
                return
            for item in self.materials_table.get_children():
                self.materials_table.delete(item)

            conn = sqlite3.connect(self.db_file)
            cursor = conn.cursor()
            cursor.execute('''
                SELECT m.material_id, m.type, m.name, s.name, m.stock_quantity, m.unit
                FROM materials m
            ...
            messagebox.showerror("Ошибка", f"Не удалось загрузить материалы: {str(e)}", parent=self)
    ...
    def test_material_calculation(self):
        try:
            ...
            material_type_id = 1
            result = calculate_material(product_type_id, material_type_id, quantity, param1, param2)
            if result == -1:
